/**
 * Database seed script to populate sentences for all grade levels
    ...
    for (const [grade, sentences] of Object.entries(sentencesByGrade)) {
      for (const sentence of sentences) {
        try {
          ...
          // Skip if sentence already exists (duplicate)
          if (error.code !== 'ER_DUP_ENTRY') {
            console.error(`Error inserting sentence for ${grade}:`, error.message);
          }
        }
      }
      console.log(`✓ Inserted ${sentences.length} sentences for grade ${grade}`);
    }

    console.log(`\n✓ Successfully inserted ${totalInserted} sentences to the database`);
  } catch (error) {
